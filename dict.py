...
#        print xml;
	print BOLD + "==========WORDS==========" + NORMAL;
	phonetic_domain = get_elements(xml, "phonetic-symbol");
	phonetic = get_text(phonetic_domain[0]);
	...
	print BOLD + UNDERLINE + queryword + NORMAL + "  [" +  phonetic.split(',')[0] + "]";
	translated = False;
	
	for cus in custom_translations:
		source = get_elements_by_path(cus, "source/name");
		
		print RED + "-------SUMMARY-------" + DEFAULT;
